"""
Tic-tac-toe for two players with GUI based on Tkinter
"""
...
class StartWindow(tk.Frame):
    ...
    def __init__(self, master: tk.Frame, controller):
        ...
        # input spot
        self.p1_text = tk.Label(self, text='Никнейм игрока X (первый):')
        self.p2_text = tk.Label(self, text='Никнейм игрока O (второй):')
        self.p1_entry = tk.Entry(self, width=21)
        self.p2_entry = tk.Entry(self, width=21)
        self.p1_entry.insert(0, 'Введите никнейм...')
        self.p2_entry.insert(0, 'Введите никнейм...')
        # confirm button
        self.confirm_btn = tk.Button(self, text='СТАРТ', width=19,
        ...
        # positioning
        self.p1_text.grid(row=1, column=0, padx=90, pady=(100, 0))
...
class BoardWindow(tk.Frame):
    def __init__(self, master: tk.Frame, controller):
        ...
        self.back_btn = tk.Button(self, text='НАЗАД', width=40)
    ...
    def write_victory(self):
        self.turn_label.configure(text=f'Победил {self.actor.name}!')
    ...
    def swap_turn(self):
        if self.player_1.is_actor:
            ...
            self.actor = self.player_1
        self.turn_label.configure(text=f'Ходит: {self.actor.name}')
...
class MainWindow:
    ...
    def write_turn(self, turn_name):
        self.board_window.turn_label.configure(text=f'Ходит: {self.board_window.actor.name}')
